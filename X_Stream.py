...
# Detect HeadPen Wrong - Right - Stream Webcam
# First Detect Pen / Stream Webcam
...
class ObjectDetection:
    ...
    def __call__(self):
        ...
        mainCounter = cv2.resize(mainCounter, (230, 100))
        ...
        while True:
            ...
            frame = cvzone.overlayPNG(frame, mainCounter, (410, 380))
            cv2.putText(frame, f'True:{frame_count_dung}', (510, 405), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 1)
            cv2.putText(frame, f'False:{frame_count_sai}', (510, 465), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1)
            cv2.putText(frame, f'FPS:{int(fps)}', (410,439), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 1)
            frame = cv2.resize(frame, (960, 540))
            cv2.imshow('X_Stream - @minhcuong-AILab', frame)
            frame_count += 1
